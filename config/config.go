...
type Config struct {
	...
	// DisableProposeBatch: if true, replicas will not batch multiple proposals into a single instance
	DisableProposeBatch bool
	...
	for s.Scan() {
		...
		default:
			...
			case "commandsize":
				c.CommandSize, err = expectInt(words)
				ok = true
			case "disableproposebatch":
				c.DisableProposeBatch, err = expectBool(words)
				ok = true
			}
			if ok {
				readingMaster = false
